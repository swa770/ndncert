...
public:
   ...
   *
   * @p fileName, the configuration file name.
  ...
  /**
   * Load CA configuration from a TLV block.
   *
   * @p configBlock, the configuration TLV.
   * @throw CaConfig::Error when the ca-prefix attribute is missing in the TLV.
   * @throw CaConfig::Error when the challenge is not specified int the TLV or is not supported.
   */
  void
  load(const Block& configBlock);

  /**
   * TLV encoding the CA config into a block used for NDNCERT INFO Data packet.
   * @return a TLV block with the type Content.
   */
  Block
  wireEncode() const;
